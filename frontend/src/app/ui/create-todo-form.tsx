...
const taskSchema = z.object({
  ...
  dueDate: z.date().optional().refine((inputDate) => {
    if (!inputDate) {
      return true;
    }
...
export function CreateTodoForm() {
  ...
  const createTodoMutation = useMutation({
    ...
    onMutate: async (newTodo) => {
      ...
      // delete based on context and temporary id if not present in cache when response arrives
      // update - create new todo and remove the temporary one from cache
      //
      // consider using randomuuid
      ...
      queryClient.setQueryData(["todos"], (old: TodoSchema[]) => {
        return old.concat({
          ...
          isCompleted: false,
          id: Math.round(Math.random() * Number.MAX_SAFE_INTEGER) * -1
        });
      });
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
        ...
        <div className="flex justify-end gap-4 text-base">
          <Button type="button" variant={"secondary"} className="text-base cursor-pointer" onClick={onClose}>
            Go back
          </Button>
